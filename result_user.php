 ...
 <div class="container">
	<div class="alert alert-primary mb-2 mt-3" role="alert">
		<?php  
			...
			echo "Hurmatli <b>".$_SESSION['fio']." ";
			echo "</b> sizning testda ko'rsatgan natijalaringiz!";
		?>
 ...
 <div class="container">
  ...
  <div class="col-3">
    <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
		...
		<?php
			$i = 1;
			$resdb = mysql_query("select * from `result` where `user_id` = '$user_id'");
			while($resmdb = mysql_fetch_array($resdb)){
				$o_id = $resmdb['otm_id'];
				$y_id = $resmdb['otm_yunalish_id'];
				 
				$otmdb = mysql_query("select * from `otm` where id = '$o_id'");
				$otmmdb = mysql_fetch_array($otmdb);
				
				$ydb = mysql_query("select * from `otm_yunalish` where id = '$y_id'");
				$ymdb = mysql_fetch_array($ydb);
				?>
		
			<a class="nav-link<?php if($i==1) {echo " active";} ?>" id="v-pills-home-tab" data-toggle="pill" href="#g<?php  echo $i; ?>" role="tab" aria-controls="v-pills-home" aria-selected="true"><?php echo $otmmdb['otm_name'];  ?>, 
				<b ><?php echo $ymdb['name'];  ?></b></a>	
